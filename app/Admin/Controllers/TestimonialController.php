<?php

namespace App\Admin\Controllers;

use App\Testimonial;

use Encore\Admin\Form;
use Encore\Admin\Grid;
use Encore\Admin\Facades\Admin;
use Encore\Admin\Layout\Content;
use App\Http\Controllers\Controller;
use Encore\Admin\Controllers\ModelForm;
use Illuminate\Support\Str;

class TestimonialController extends Controller
{
    use ModelForm;

    /**
     * Index interface.
     *
     * @return Content
     */
    public function index()
    {
        return Admin::content(function (Content $content) {

            $content->header('Testimonial');

            $content->body($this->grid());
        });
    }

    /**
     * Edit interface.
     *
     * @param $id
     * @return Content
     */
    public function edit($id)
    {
        return Admin::content(function (Content $content) use ($id) {

            $content->header('Testimonial');

            $content->body($this->form()->edit($id));
        });
    }

    /**
     * Create interface.
     *
     * @return Content
     */
    public function create()
    {
        return Admin::content(function (Content $content) {


            $content->header('Testimonial');

            $content->body($this->form());
        });
    }

    /**
     * Make a grid builder.
     *
     * @return Grid
     */
    protected function grid()
    {
        return Admin::grid(Testimonial::class, function (Grid $grid) {

            $grid->id('ID')->sortable();
            $grid->author('Author')->sortable();
            $grid->city('City')->sortable();
            $grid->description('Description')->sortable()->display(function($text) {
                return Str::limit(strip_tags($text), 100, '...');
            });
            $grid->created_at();
            $grid->updated_at();
        });
    }

    /**
     * Make a form builder.
     *
     * @return Form
     */
    protected function form()
    {
        return Admin::form(Testimonial::class, function (Form $form) {
            $form->display('id', 'ID');
            $form->text('author', 'Author')->rules('required|string|min:3|max:200');
            $form->text('city', 'City')->rules('required|string|min:3|max:200');
            $form->ceditor('description', 'Description')->rules('required|string|min:3|max:10000');
            $form->display('created_at', 'Created At');
            $form->display('updated_at', 'Updated At');
        });
    }
}
